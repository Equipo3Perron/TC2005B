    ...
    try {
      if (
        ...
        const result = await pool
          ...
          .query(
            'insert into [dbo].[Usuario] values(@nombre, @apellido, @email, @pass, @admin) EXECUTE SPUpdatePlayer 0, 0, @nombre, 3000, 0, 0, 0 , 1 ,True'
          );
        res.json(result);
      } else {
  ...
  async login(req, res) {
    try {
      ...
      const result = await pool
        ...
        .query(
          'SELECT Email, Password, EsAdministrador, IDUsuario FROM [dbo].[Usuario] WHERE Email = @email'
        );
